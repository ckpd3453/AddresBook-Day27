...
public class Services13 {
	/**
	 * creating person object of ServerBook13 class
	 */
	ServerBook13 person = new ServerBook13();

	/**
	 * Creating a List of ServerBook13 using ArrayList
	 */
	List<ServerBook13> contactDetailsList = new ArrayList<>();
	...
	public void writeContact() {
		...
		/**
		 * storing or adding all the ServerBook13 to the person
		 */
		person = new ServerBook13(firstName, lastName, address, city, state, zipCode, mobileNumber, emailId);
	...
	public void searchByName(String name) {
		...
		/**
		 * creating Stream from list of ServerBook13. Filter operation produces a new
		 ...
		 * and will collect in a ServerBook13 using collector
		 */
		List<ServerBook13> collect = contactDetailsList.stream().filter(p -> p.getFirstName().equalsIgnoreCase(name))
				.collect(Collectors.toList());
		for (ServerBook13 contact : collect) {
	...
	public void searchByCity(String city) {
		List<ServerBook13> collect = contactDetailsList.stream().filter(p -> p.getCity().equalsIgnoreCase(city))
				.collect(Collectors.toList());
	...
	public void searchByState(String state) {
		List<ServerBook13> collect = contactDetailsList.stream().filter(p -> p.getCity().equalsIgnoreCase(state))
				.collect(Collectors.toList());
		for (ServerBook13 contact : collect) {
			System.out.println("Search result: " + contact);
		}
	...
	public void countContactsByUsingCity(String cityName) {
		...
		for (ServerBook13 contact : contactDetailsList) {
			count1 = count1 + count;
		}
	...
	public void sortByName() {
		List<ServerBook13> list = contactDetailsList.stream().collect(Collectors.toList());
